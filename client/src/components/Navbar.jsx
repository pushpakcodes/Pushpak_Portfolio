...
const Navbar = () => {
  ...
  const isBlogsPage = currentPath === '/blogs' || currentPath.startsWith('/blogs/');
  ...
  return (
    <nav className="fixed top-4 md:top-8 left-1/2 -translate-x-1/2 z-50">
      <div className={`flex items-center gap-1 md:gap-2 backdrop-blur-md border rounded-full p-1 md:p-1.5 px-2 md:px-3 shadow-lg transition-colors duration-300 ${
        isBlogsPage
          ? 'bg-[#FAF8F5]/90 border-[#E5DFD5] shadow-stone-900/10'
          : isArtPage 
        ...
        {navItems.map((item) => (
          <div 
            ...
            className={`
              ...
              ${isActive(item.path) 
                ? isBlogsPage
                  ? 'bg-black/10 text-black hover:bg-black/20 shadow-sm font-bold'
                  : isArtPage
                ...
                : isBlogsPage
                  ? 'text-stone-600 hover:text-black font-medium'
                  : isArtPage
        ...
        <a 
          href="https://drive.google.com/file/d/1En8E4U8o5nWJdEstaw2lTjkqDNQdTB-B/view?usp=sharing" 
          target="_blank" 
          ...
          className={`
            ...
            ${isBlogsPage
              ? 'bg-black/10 hover:bg-black/20 text-black border-black/10 font-bold'
              : isArtPage
